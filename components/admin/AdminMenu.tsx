...
export default function AdminMenu({user} : {user: User}) {
  ...
  return (
    <Popover className="relative">
      ...
      >
        <PopoverPanel className="absolute left-1/2 z-10 mt-5 flex w-screen lg:max-w-min -translate-x-1/2 lg:-translate-x-48 text-center">
          <div className="w-full lg:w-56 shrink rounded-xl bg-white p-4 text-sm font-semibold leading-6 text-gray-900 shadow-lg ring-1 ring-gray-900/5">
            <p className='text-center'>Hola: {user.name}</p>
            <Link
                href='/admin/profile/settings'
                className='block p-2 hover:text-purple-800 transition-colors'
            >Mi Perfil</Link>
            <Link
                href='/admin'
                className='block p-2 hover:text-purple-800 transition-colors'
